"""Vector ECG strip: clinical grid (25 mm/s, 1 mm minor / 5 mm major), one lane
per lead, gain auto-picked so the tallest selected lead still fits its lane."""
...
def _auto_gain(sig, pick, row_mm: float) -> float:
...
def _grid(parts: list[str], w_mm: float, h_mm: float, px_mm: float) -> None:
    W, H = w_mm * px_mm, h_mm * px_mm
    for step, colour, stroke in GRID:
        x = 0.0
        while x <= w_mm + 1e-6:
            parts.append(
                f'<line x1="{x * px_mm:.1f}" y1="0" x2="{x * px_mm:.1f}" '
                f'y2="{H:.1f}" stroke="{colour}" stroke-width="{stroke}"/>'
            )
            x += step
        y = 0.0
        while y <= h_mm + 1e-6:
            parts.append(
                f'<line x1="0" y1="{y * px_mm:.1f}" x2="{W:.1f}" '
...
def to_svg(
    ...
    parts = [
        ...
        f'viewBox="0 0 {W:.1f} {H:.1f}"><rect width="100%" height="100%" fill="#fff"/>'
    ]
    ...
    for row, ch in enumerate(pick):
        ...
        s = sig[ch]
        coarse = s[::7]
        med = float(sorted(coarse)[len(coarse) // 2])  # lane baseline
        pts = []
        for i in range(n):
            x = (i / rec.fs) * mm_s * px_mm
            y = (base_mm - (s[i] - med) * mm_mv) * px_mm
    ...
    parts.append(
        f'<text x="{W - 250:.0f}" y="{H - 6:.0f}" font-family="monospace" '
