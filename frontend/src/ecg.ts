...
export function createEcgView(h: EcgHandlers): EcgView {
  ...
  let curStart = 0,
    curDur = 10,
    cursorSec: number | null = null,
    curIso = "";
  let cursorNode: SVGGElement | null = null,
    rangeNode: SVGRectElement | null = null;
  ...
  window.addEventListener("mousemove", (e) => {
    if (sel) {
      const a = secAt(sel.x0),
        b = secAt(e.clientX);
      range = { t0: Math.min(a, b), t1: Math.max(a, b) };
      drawRange();
      return;
    }
    if (!hand) return;
    ...
    if (hand.moved) {
      const r = root.getBoundingClientRect();
      h.onPan(hand.start0 - (dx / r.width) * curDur - curStart, true);
    }
  });
  window.addEventListener("mouseup", (e) => {
    if (sel) {
      sel = null;
      if (range && range.t1 - range.t0 >= 0.1) h.onRange(range.t0, range.t1);
      else {
        range = null;
        drawRange();
      }
    ...
    if (!hand) return;
    const wasClick = hand.moved === false;
    hand = null;
    root.style.cursor = "";
    if (wasClick) h.onCursor(secAt(e.clientX));
  ...
  return {
    ...
    render(win, startIso, gainOpt) {
      ...
      const pxS = W / dur; // пикселей на секунду
      const mm = pxS / 25; // пикселей на мм при 25 мм/с
      ...
      for (const q of win.quality_manual) {
        const a = Math.max(0, (q.t0_ms / 1000 - win.start) * pxS),
          b = Math.min(W, (q.t1_ms / 1000 - win.start) * pxS);
        if (b <= a) continue;
        ...
        t.textContent = q.v === "clean" ? "чисто · вручную" : "помеха · вручную";
        s.append(t);
      }
      ...
      win.data.forEach((row, li) => {
        ...
        for (let i = 0; i < row.length; i++) {
          const x = (i / win.fs) * pxS,
            y = base - row[i] * pxMv;
          d += (i ? "L" : "M") + x.toFixed(1) + " " + y.toFixed(1);
        }
        const noisy = win.lead_noise[li] > 2.5,
          drifting = win.lead_drift[li] > win.drift_thr_mv;
        s.append(svg("path", { class: `trace${noisy || drifting ? " noisy" : ""}`, d }));
        const name = svg("text", { class: "lead-name", x: 6, y: MARK_H + lane * li + 13 });
        name.textContent = win.leads[li];
        s.append(name);
        const nz = svg("text", { class: "lead-noise", x: 6, y: MARK_H + lane * li + 25 });
        nz.textContent = [
          noisy ? `шум ×${win.lead_noise[li].toFixed(1)}` : "",
          drifting ? `дрейф ${win.lead_drift[li].toFixed(1)} мВ` : "",
        ]
          .filter(Boolean)
          .join(" · ");
        s.append(nz);
      ...
      for (const b of win.beats) {
        ...
        t.textContent = b.added ? `+${b.label}` : b.manual ? `${b.manual}·` : b.label;
        g.append(t);
      ...
      for (const sec of events) {
        ...
        const t = svg("text", { class: "evlbl", x: ex + 4, y: MARK_H + 36 });
        t.textContent = "симптом";
        s.append(t);
      }
      // курсор времени для вставки комплекса
      ...
      drawCursor();
      drawRange();
      // масштабная метка: 1 с × 1 мВ
      const sc = svg("g", { class: "scale" });
      const sx = W - pxS - 14,
        sy = H - PAD_B - 8;
      sc.append(svg("line", { x1: sx, x2: sx + pxS, y1: sy, y2: sy }));
      sc.append(svg("line", { x1: sx, x2: sx, y1: sy, y2: sy - pxMv }));
      const st = svg("text", { x: sx + 4, y: sy - 4 });
      st.textContent = `1 с · 1 мВ · ${gain} мм/мВ`;
      sc.append(st);
      s.append(sc);

      // подписи времени каждую секунду
      ...
      for (let sec = 0; sec <= dur; sec += step) {
        ...
        t.textContent = clock(startIso, win.start + sec);
        tl.append(t);
      }
      s.append(tl);
    },
