...
import { clock, el, LABEL_RU, svg, VERDICT_RU } from "./util";
...
export function createTemplates(
  ...
  function card(sc: Scored): HTMLElement {
    ...
    const dev = Object.entries(t.labels)
      .sort((a, b) => b[1] - a[1])
      .map(([k, v]) => `${k} ${v}`)
      .join(" · ");
    ...
    if (Object.keys(t.manual).length)
      verdictTxt = `врач: ${Object.entries(t.manual)
        .map(([k, v]) => `${k} ${v}`)
        .join(" · ")}`;
    else if (sc.nV === 0)
      verdictTxt = (t.labels.S ?? 0) ? `форма синусовая; ${t.labels.S} ранних (НЖЭС) по времени` : "синусовая форма";
    ...
    l0.textContent = clock(startIso, 0).slice(0, 5);
    const l1 = svg("text", { x: CARD_W - 1, y: 19.5, class: "tpl-hours-lbl", "text-anchor": "end" });
    l1.textContent = clock(startIso, hrs.length * 3600).slice(0, 5);
    sp.append(l0, l1);
    const when = el("div", "tpl-when");
    when.append(sp, el("span", "mono muted", hoursNote(hrs)));
    c.append(when);
