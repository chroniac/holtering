...
const PAGE_W = 182; // печатная ширина, мм (A4 минус поля 14 мм)
const STRIP_W = 180; // 7,2 с при 25 мм/с
const LANE = 16; // мм на отведение
...
export function createReport(defaultLeads: string[], onJump: (sec: number, dur: number) => void): ReportView {
  ...
  const metaField = (k: string, placeholder: string) =>
    editable("span", "meta", saved.meta[k] ?? "", placeholder, (v) => {
  ...
  function trendSvg(): SVGElement {
    ...
    for (let hh = 0; hh <= hours; hh += 2) {
      ...
      t.textContent = clock(sum.record.start, hh * 3600).slice(0, 5);
      sv.append(t);
    }
    let d = "",
      pen = false;
  ...
  function stripBlock(spec: StripSpec & { auto: boolean }, win: EcgWindow, idx: number): HTMLElement {
    ...
    cap.append(
      editable("span", "cap", capText, "подпись", (v) => {
        if (spec.auto) {
  ...
  function renderSide(visible: (StripSpec & { auto: boolean })[]) {
    ...
    for (const s of visible) {
      ...
      row.append(
        el("span", "mono", clock(sum.record.start, s.t0_ms / 1000).slice(0, 5)),
        el("span", "rs-cap", (s.auto ? saved.captions[s.id] : undefined) ?? s.caption),
      );
  ...
  async function render() {
    const autos: (StripSpec & { auto: boolean })[] = data.auto_strips
      .filter((s) => !saved.hidden.includes(s.id))
      .map((s) => ({ ...s, auto: true }));
  ...
  return {
    ...
    async addStrip(t0_s, dur_s, stripLeads) {
      ...
      if (!saved.strips.some((s) => s.id === id))
        saved.strips.push({
          id,
          kind: "manual",
