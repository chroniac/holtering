...
export const createMinimap = (
  ...
  let data: BeatsWindow | null = null,
    loaded: Range | null = null;
  let events: number[] = [];
  let W = 1000;
  ...
  const draw = () => {
    ...
    for (const nw of data.noise_windows) {
      const a = Math.max(0, x(nw.t0)),
        b = Math.min(W, x(nw.t1));
      if (b > a) c2.fillRect(a, 0, b - a, AXIS_Y);
    }
    c2.strokeStyle = "#f0dcdc";
    c2.lineWidth = 1;
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (px < 0 || px > W) continue;
      c2.strokeStyle = CLS_COLOR[c];
      c2.lineWidth = c === 3 ? 1 : 1.5;
      c2.beginPath();
      c2.moveTo(px, TICK_Y);
      c2.lineTo(px, TICK_Y + TICK_H);
      c2.stroke();
    }
    for (const sec of events) {
      ...
      c2.closePath();
      c2.fill();
      c2.strokeStyle = "#6f5fa8";
      c2.setLineDash([3, 3]);
      c2.beginPath();
      c2.moveTo(px + 0.5, 12);
      c2.lineTo(px + 0.5, AXIS_Y);
      c2.stroke();
      c2.setLineDash([]);
    }
    const a = x(strip.start),
      b = Math.max(a + 3, x(strip.start + strip.dur));
    c2.fillStyle = "rgba(200,16,46,0.10)";
    c2.fillRect(a, 0, b - a, AXIS_Y);
    c2.strokeStyle = "#c8102e";
    c2.lineWidth = 1;
    c2.strokeRect(Math.round(a) + 0.5, 0.5, Math.round(b - a), AXIS_Y - 1);
    c2.fillStyle = "#c8102e";
  ...
  cv.addEventListener("mousemove", (e) => {
    if (!drag) cv.style.cursor = cursorFor(hit(pxOf(e)));
  });
  cv.addEventListener("mousedown", (e) => {
    const p = pxOf(e),
      mode = hit(p);
    if (mode === "new") {
      onStrip(clampRange({ start: sec(p) - strip.dur / 2, dur: strip.dur }, total), false);
      return;
    }
    drag = { mode, px: p, r: { ...strip } };
    cv.style.cursor = "grabbing";
    e.preventDefault();
  });
  window.addEventListener("mousemove", (e) => {
    if (!drag) return;
    const ds = ((pxOf(e) - drag.px) / W) * ctx.dur,
      r = drag.r;
    if (drag.mode === "move") onStrip(clampRange({ start: r.start + ds, dur: r.dur }, total), true);
    else if (drag.mode === "left") {
      const nd = Math.max(STRIP_MIN, Math.min(STRIP_MAX, r.dur - ds));
      onStrip(clampRange({ start: r.start + r.dur - nd, dur: nd }, total), true);
    } else if (drag.mode === "right")
      onStrip(clampRange({ start: r.start, dur: Math.max(STRIP_MIN, Math.min(STRIP_MAX, r.dur + ds)) }, total), true);
  });
  ...
  cv.addEventListener(
    ...
    (e) => {
      ...
      if (w.kind === "pan")
        onStrip(
          clampRange({ start: strip.start + Math.sign(w.delta) * strip.dur * 0.5, dur: strip.dur }, total),
          false,
        );
