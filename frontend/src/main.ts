...
import { STRIP_MAX, STRIP_MIN, createMinimap } from "./minimap";
import { type Range, clampRange, zoomAround } from "./nav";
import { CTX_MAX, CTX_MIN, renderOverview } from "./overview";
import { createReport } from "./report";
import { createTemplates } from "./templates";
import { clock, el } from "./util";

/** Limb block is channels 0-5 (proved by the Einthoven identities in scp_holter.leads);
 *  everything after is a chest channel whatever it is labelled (ch6.. or V1.. via --chest). */
const LIMB_COUNT = 6;
function leadSets(all: string[]) {
  const limb = all.slice(0, LIMB_COUNT), chest = all.slice(LIMB_COUNT);
  // default trio: II plus the two chest channels nearest the classic V2/V5 positions
  // (5th and 2nd chest channel, in stored order), falling back to whatever exists
  const trio = [limb[1] ?? all[0], chest[4] ?? chest[chest.length - 1], chest[1] ?? chest[0]].filter((x, i, arr) => x && arr.indexOf(x) === i);
...
/** Four stat cards, left of the overview: hero number + note, then aligned key/value rows. */
function statsBlock(sum: Summary): HTMLElement {
  const box = el("div", "cards");
  const c = sum.counts, h = sum.hr, r = sum.record;
  const card = (title: string, hero: string, note: string, rows: [string, string][], bar?: HTMLElement) => {
    ...
    const hv = el("div", "hero mono"); hv.textContent = hero;
    if (note) { const hn = el("span", "hero-note mono"); hn.textContent = note; hv.append(hn); }
    d.append(hv);
    if (bar) d.append(bar);
    const kv = el("div", "kv");
    for (const [k, v] of rows) { const row = el("div", "kvr"); row.innerHTML = `<span>${k}</span><b class="mono">${v}</b>`; kv.append(row); }
    d.append(kv);
    box.append(d);
  };
  const segbar = (parts: [string, number][]) => {
    const total = Math.max(1, parts.reduce((p, [, n]) => p + n, 0));
    const bar = el("div", "segbar");
    for (const [cls, n] of parts) { if (n > 0) { const sg = el("i", `seg ${cls}`); sg.style.flex = String(n / total); sg.title = `${n}`; bar.append(sg); } }
    return bar;
  };
  const vv = c.verdicts.V ?? {}, sv = c.verdicts.S ?? {};
  const rej = (o: Record<string, number>) => Object.entries(o).filter(([k]) => k !== "likely" && k !== "uncertain" && k !== "manual").reduce((p, [, n]) => p + n, 0);
  const vL = c.audited.V_likely, vU = c.audited.V - vL, vR = rej(vv);
  const sL = c.audited.S, sR = rej(sv);
  ...
  card("ЖЭС", `${vL}–${c.audited.V}`, `${(100 * c.audited.V / r.beats).toFixed(2)}%`, [
    ["прибор насчитал", `${c.device.V}`],
    ["вероятные · спорные", `${vL} · ${vU}`],
    ["отклонено", `${vR}`],
    ["морфологий", nm ? `${nm}` : "—"],
  ], segbar([["likely", vL], ["uncertain", vU], ["rejected", vR]]));
  card("НЖЭС", `${sL}`, `${(100 * sL / r.beats).toFixed(2)}%`, [
    ["прибор насчитал", `${c.device.S}`],
    ["отклонено", `${sR}`],
  ], segbar([["likely", sL], ["rejected", sR]]));
...
function tabs<T extends string>(items: [T, string][], initial: T, onPick: (t: T) => void): { root: HTMLElement; set(t: T): void; label(t: T, s: string): void } {
  const root = el("div", "tabs");
  const btns: Record<string, HTMLElement> = {};
  for (const [id, label] of items) {
    const b = el("button", `tab${id === initial ? " on" : ""}`, label);
    b.addEventListener("click", () => onPick(id));
    btns[id] = b; root.append(b);
  }
  return { root, set(t) { for (const [k, b] of Object.entries(btns)) b.classList.toggle("on", k === t); }, label(t, s) { btns[t].textContent = s; } };
}
...
async function boot() {
  ...
  // navigation state: context (overview → minimap) and strip (minimap → ECG)
  ...
  let mainTab: MainTab = "strip", sideTab: SideTab = "episodes";

  // ---- navbar: brand | tabs (centred) | legend · summary toggle · help --------
  const hdr = el("header", "hdr");
  const brand = el("div", "brand"); brand.append(el("span", "logo", "holtering"));
  const mainTabs = tabs<MainTab>([["strip", "Лента"], ["disclosure", "Раскрытие"], ["templates", "Шаблоны"], ["report", "Заключение"]], "strip", (t) => setMain(t));
  ...
  const topBtn = el("button", "ibtn", "сводка"); topBtn.title = "показать / скрыть сводку";
  const help = el("button", "ibtn", "?"); help.title = "клавиши";
  right.append(legend, topBtn, help);
  hdr.append(brand, mainTabs.root, right);
  app.append(hdr);

  // ---- top: record line + stats cards + overview -----------------------------
  ...
  const ovView = renderOverview(sum, ov, episodes, (c, live) => setContext(c, live), (ep) => gotoEpisode(ep));
  top.append(left, ovView.root);
  app.append(top);
  let topOpen = localStorage.getItem("holtering.top") !== "0";
  const applyTop = () => { top.hidden = !topOpen || mainTab === "templates"; topBtn.classList.toggle("on", topOpen); localStorage.setItem("holtering.top", topOpen ? "1" : "0"); };
  topBtn.addEventListener("click", () => { topOpen = !topOpen; applyTop(); redraw(); });

  // ---- main area -----------------------------------------------------
  const main = el("div", "main");
  const area = el("div", "area");
  const stripPane = el("div", "pane strip");
  // toolbar: three labelled groups. Время (where am I, step) | Окно (how much, how tall) | Отведения (what)
  const bar = el("div", "ecg-bar");
  const group = (label: string, items: HTMLElement[], labelRowExtra?: HTMLElement) => {
    const g = el("div", "tb-group");
    const lr = el("div", "tb-labelrow"); lr.append(el("div", "tb-label", label)); if (labelRowExtra) lr.append(labelRowExtra);
    g.append(lr);
    const row = el("div", "tb-row"); row.append(...items); g.append(row);
    return g;
  };
  // time: editable clock (Enter jumps), duration next to it, step buttons with explicit meaning
  ...
  timeIn.addEventListener("keydown", (e) => {
    ...
    const st = new Date(startIso.replace(" ", "T")); const d = new Date(st); d.setHours(hh, mm, ss || 0, 0);
    let sec = (d.getTime() - st.getTime()) / 1000; if (sec < 0) sec += 86400;
  ...
  prev.title = "окно назад  ←"; next.title = "окно вперёд  →";
  const stepSeg = el("div", "seg"); stepSeg.append(prev, next);
  const prevEp = el("button", "btn", "‹ эпизод"), nextEp = el("button", "btn", "эпизод ›");
  prevEp.title = "предыдущий эпизод  ["; nextEp.title = "следующий эпизод  ]";
  const epSeg = el("div", "seg"); epSeg.append(prevEp, nextEp);
  const toRep = el("button", "btn tiny", "в заключение"); toRep.title = "добавить это окно в фрагменты ЭКГ протокола  p";
  const addToReport = async () => {
    await rep.addStrip(strip.start, strip.dur, leads);
    toRep.textContent = "добавлено"; toRep.classList.add("on");
    window.setTimeout(() => { toRep.textContent = "в заключение"; toRep.classList.remove("on"); }, 1200);
  };
  toRep.addEventListener("click", () => void addToReport());
  const gTime = group("Время", [timeIn, time, stepSeg, epSeg], toRep);

  // window: duration and gain, each as its own labelled segment
  ...
  for (const g of ["auto", "5", "10", "20"]) {
    ...
    b.addEventListener("click", () => { gain = g === "auto" ? "auto" : Number(g); for (const [k, x] of Object.entries(gainBtns)) x.classList.toggle("on", k === g); redraw(); });
    gainBtns[g] = b; gainSeg.append(b);
  }
  const gWin = group("Окно", [el("span", "tb-sub", "секунд"), durSeg, el("span", "tb-sub", "мм/мВ"), gainSeg]);

  // leads: presets first, then individual toggles
  const PRESETS: [string, string[]][] = [["3", sets.trio], ["конечн.", sets.limb], ["грудные", sets.chest], ["все", sets.all]];
  const presetSeg = el("div", "seg");
  const leadBox = el("div", "leads");
  const leadBtns: Record<string, HTMLElement> = {};
  const syncLeads = () => {
    if (!leads.length) leads = [sum.record.leads[0]];           // a preset that matched nothing must not blank the strip
    for (const [k, x] of Object.entries(leadBtns)) x.classList.toggle("on", leads.includes(k));
    void load();
  };
  for (const [name, set] of PRESETS) {
    const b = el("button", "btn", name); b.title = set.join(", ");
    b.addEventListener("click", () => { leads = sum.record.leads.filter((x) => set.includes(x)); syncLeads(); });
    presetSeg.append(b);
  }
  for (const name of sum.record.leads) {
    const b = el("button", `btn${leads.includes(name) ? " on" : ""}`, name);
    b.addEventListener("click", () => {
      if (leads.includes(name)) { if (leads.length === 1) return; leads = leads.filter((x) => x !== name); }
      else leads = sum.record.leads.filter((x) => leads.includes(x) || x === name);
      syncLeads();
    });
    leadBtns[name] = b; leadBox.append(b);
  }
  ...
  const fd = createDisclosure(startIso, total, sum.record.leads, (sec) => { setMain("strip"); setStrip({ start: sec - strip.dur / 2, dur: strip.dur }, false); });
  fd.root.classList.add("pane"); fd.root.hidden = true;
  const tpl = createTemplates(startIso, sum.record.leads, (idx) => { setMain("strip"); void gotoBeat(idx); }, () => refreshAll());
  tpl.root.classList.add("pane"); tpl.root.hidden = true;
  const rep = createReport(sets.trio, (sec, dur) => { setMain("strip"); setStrip({ start: sec, dur }, false); });
  rep.root.classList.add("pane"); rep.root.hidden = true;
  area.append(stripPane, fd.root, tpl.root, rep.root);

  // ---- sidebar -------------------------------------------------------
  const side = el("aside", "side");
  const sideTabs = tabs<SideTab>([["episodes", `Эпизоды · ${episodes.length}`], ["diary", "Симптомы"], ["beat", "Комплекс"]], "episodes", (t) => setSide(t));
  let epView = renderEpisodes(sum, episodes, (ep) => gotoEpisode(ep));
  const diary = createDiary(sum, (sec) => { setMain("strip"); setStrip({ start: sec - strip.dur / 2, dur: strip.dur }, false); }, () => { void refreshEvents(); });
  diary.root.hidden = true;
  const beatPanel = createBeatPanel({
    onLabel: (l) => void label(l),
    onAdd: (l) => void addBeat(l),
    onFamily: (t) => { setMain("templates"); void tpl.open(t); },
  ...
  const keys = el("div", "keys"); keys.hidden = true;
  keys.innerHTML = `<b>Навигация</b> shift+колесо: влево/вправо · ctrl+колесо: масштаб · тянуть ленту: сдвиг · ← →: окно, с shift ×6 · − +: масштаб<br>` +
  ...
  function setMain(t: MainTab) {
    mainTab = t; mainTabs.set(t);
    stripPane.hidden = t !== "strip"; fd.root.hidden = t !== "disclosure"; tpl.root.hidden = t !== "templates"; rep.root.hidden = t !== "report";
    applyTop();                                // the 24 h timeline is meaningless while relabelling families
  ...
  function setSide(t: SideTab) {
    sideTab = t; sideTabs.set(t);
    epView.root.hidden = t !== "episodes"; beatPanel.root.hidden = t !== "beat"; diary.root.hidden = t !== "diary";
  ...
  async function refreshEvents() {
    ...
    ovView.setEvents(eventTimes); mini.setEvents(eventTimes); ecg.setEvents(eventTimes);
    sideTabs.label("diary", evs.length ? `Симптомы · ${evs.length}` : "Симптомы");
    redraw();
  }

  let last: EcgWindow | null = null;
  let reqId = 0, liveTimer: number | null = null;
  function paint() {
  ...
  async function load() {
    ...
    ecg.render(win, startIso, gain); ecg.setCursor(cursorSec); ecg.setRange(range);
    if (activeBeat) {
      const fresh = win.beats.find((b) => b.index === activeBeat!.index);
      if (fresh) selectBeat(fresh, false); else ecg.highlight(activeBeat.index);
    }
  }
  function redraw() { if (last && mainTab === "strip") { ecg.render(last, startIso, gain); ecg.setCursor(cursorSec); ecg.setRange(range); ecg.highlight(activeBeat?.index ?? null); } }
  /** Strip moved: keep it inside the context (context pages along when it leaves). */
  function setStrip(r: Range, live: boolean) {
    strip = clampRange({ start: r.start, dur: Math.round(Math.max(STRIP_MIN, Math.min(STRIP_MAX, r.dur)) * 10) / 10 }, total);
    if (strip.start < ctx.start || strip.start + strip.dur > ctx.start + ctx.dur) {
      ctx = clampRange({ start: strip.start + strip.dur / 2 - ctx.dur / 2, dur: Math.max(ctx.dur, strip.dur * 3) }, total);
    }
    paint();
    if (liveTimer) window.clearTimeout(liveTimer);
    if (live) liveTimer = window.setTimeout(() => void load(), 120); else void load();
  }
  /** Context changed on the overview: strip follows into it (centred) if it fell outside. */
  function setContext(c: Range, live: boolean) {
    ...
    if (strip.start < ctx.start || strip.start + strip.dur > ctx.start + ctx.dur) {
      ...
      if (liveTimer) window.clearTimeout(liveTimer);
      if (live) { paint(); liveTimer = window.setTimeout(() => void load(), 150); } else void load();
    } else paint();
  }
  function gotoEpisode(ep: Episode) {
    ...
    activeBeat = null; setCursor(null);
    ovView.setActiveEpisode(ep.id); epView.setActive(ep.id);
    setStrip({ start: ep.t_ms / 1000 - (dur - epDur) / 2, dur }, false);
    window.setTimeout(() => { const b = last?.beats.find((x) => x.index === ep.beats[0]); if (b) selectBeat(b, false); }, 200);
  }
  async function gotoBeat(idx: number) {
    const b = await api.beat(idx);
    activeEp = null; ovView.setActiveEpisode(null); epView.setActive(null);
    activeBeat = b; setCursor(null);
    setStrip({ start: b.t_ms / 1000 - strip.dur / 2, dur: strip.dur }, false);
    setSide("beat");
  }
  function selectBeat(b: Beat, focusPanel: boolean) {
    activeBeat = b; cursorSec = null; ecg.setCursor(null); range = null; ecg.setRange(null);
    ecg.highlight(b.index); beatPanel.showBeat(b, startIso);
    if (focusPanel) setSide("beat");
  }
  function setRange(r: { t0: number; t1: number } | null) {
    range = r; ecg.setRange(r);
    if (r) { activeBeat = null; cursorSec = null; ecg.highlight(null); ecg.setCursor(null); setSide("beat"); }
  ...
  function setCursor(sec: number | null) {
    cursorSec = sec;
    if (sec !== null && range) { range = null; ecg.setRange(null); }
    if (sec !== null) { activeBeat = null; ecg.highlight(null); beatPanel.showBeat(null, startIso); setSide("beat"); }
    ecg.setCursor(sec); beatPanel.showCursor(sec, startIso);
  }
  ...
  async function addBeat(l: ManualLabel) {
    ...
    try {
      ...
      cursorSec = null; ecg.setCursor(null); activeBeat = b;
      await refreshAll();
    } catch (e) {
  ...
  async function refreshAll() {
    ...
    const ns = statsBlock(sum); stats.replaceWith(ns); stats = ns;
    ovView.update(ov, episodes); ovView.setContext(ctx, strip); ovView.setActiveEpisode(activeEp);
    sideTabs.label("episodes", `Эпизоды · ${episodes.length}`);
    const nv = renderEpisodes(sum, episodes, (ep) => gotoEpisode(ep));
    epView.root.replaceWith(nv.root); epView = nv; epView.root.hidden = sideTab !== "episodes"; epView.setActive(activeEp);
  ...
  function stepEpisode(dir: 1 | -1) {
    ...
    if (i === -1) {
      i = dir > 0 ? episodes.findIndex((e) => e.t_ms / 1000 > strip.start + strip.dur) : episodes.map((e) => e.t_ms / 1000 < strip.start).lastIndexOf(true);
      if (i === -1) return;
  ...
  window.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Escape") { activeBeat = null; ecg.highlight(null); setCursor(null); setRange(null); beatPanel.showBeat(null, startIso); }
    ...
    else if (e.key === "ArrowLeft") setStrip({ start: strip.start - (e.shiftKey ? strip.dur * 6 : strip.dur), dur: strip.dur }, false);
    else if (e.key === "ArrowRight") setStrip({ start: strip.start + (e.shiftKey ? strip.dur * 6 : strip.dur), dur: strip.dur }, false);
    ...
    else if (e.key === "+" || e.key === "=") setStrip(zoomAround(strip, strip.start + strip.dur / 2, 1 / 1.5, STRIP_MIN, STRIP_MAX), false);
  ...
  else if (first) gotoEpisode(first); else void load();
}
